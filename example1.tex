...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[trim axis left, trim axis right] % options to centre correctly
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Slope field of $\frac{\mathrm{d}}{\mathrm{d}t}=3-0.08w$, with general solution $w(t)=Ce^{-0.08t}+37.5$}
\end{figure}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[trim axis left, trim axis right] % options to centre correctly
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Slope field of $\frac{\mathrm{d}y}{\mathrm{d}x}=k(2x+1)$, with general solution $y(t)=k(x^2+x)$}
\end{figure}
...
\begin{figure}[h]
    ...
    \begin{tikzpicture}[trim axis left, trim axis right] % options to centre correctly
        ...
        \begin{axis}[view = {0}{90}, % set camera to point towards x-y plane
            ...
            % plot unit length quivers with no head
            \addplot3[black!60, quiver={u=1/(sqrt((\dydx)^2+1)), v=(\dydx)/(sqrt((\dydx)^2+1)), scale arrows=0.08}, samples=32, forget plot] (x,y,0);
        ...
        \end{axis}
    \end{tikzpicture}
    \caption{Slope field of $\frac{\mathrm{d}y}{\mathrm{d}x}-\frac{y}{x}=-\frac{2}{x}$, with general solution $y(t)=Ax+2$}
\end{figure}
