...
\begin{figure}[h]
	...
	% actual axis for plotting the solution, must come after slope field to provide proper masking
	    ...
	    \pgfplotsinvokeforeach{0,...,\totalSlopes} {
		    % \edef\blah{5}
		    % \pgfmathparse{2*5}\edef\blah{\pgfmathresult}
		    ...
		    \pgfmathparse{\scale*\slope/sqrt((\slope)^2+1)}\edef\dy{\pgfmathresult}
		    \edef\tmp{\noexpand\draw[teal] ({\hx+\xmin+\i*\hx-\dx/2},{\hy+\ymin+\j*\hy-\dy/2})--({\hx+\xmin+\i*\hx+\dx/2},{\hy+\ymin+\j*\hy+\dy/2});}\tmp
		    % \edef\tmp{\noexpand\draw ({\hx+\xmin+\i*\hx},{\hy+\ymin+\j*\hy})--({\hx+\xmin+\i*\hx},{\hy+\ymin+\j*\hy+2});}\tmp
	    }
